...
library(tweedie) 
library(ggplot2)
library(doParallel) # Load a library for doing parallel computning

maxcores <- 8
Cores <- min(parallel::detectCores(), maxcores) # Find the number of available cores

cl <- makeCluster(Cores)
registerDoParallel(cl) # Initiate the number of cores we would use
...
MTweedieTests <-  
  function(N,M,sig){
    results <- foreach(m = 1:M,                              # Uses the foreach function to use parallel computing
                       .combine='c',                         # Each "loop" is added as a vector
                       .packages = "tweedie",                # Initiate necessary libraries 
                       .export = "simTweedieTest") %dopar% { # Initiate necessary global functions
        simTweedieTest(N)
    }
    sum(results < sig) / M
